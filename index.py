  ...
  try:
    ...
    if action == 'login':
        ...
        if username ==  ADMIN_USER and password == ADMIN_PASS:
          ...
          return show_login_page(error="❌ Invalid password")
    elif action == 'logout':
      ...
      # Check authentication for all other actions
        ...
        return show_login_page(error="🔒 Session expired, please login again")
  ...
  <body>
      <div class="login-card">
          ...
          <form method="get" action="">
              ...
              <button type="submit" class="btn">🚀 Access VPN Dashboard</button>
          </form>

          <div class="features">
              <div class="feature">📱<br>Mobile Friendly</div>
              <div class="feature">🔐<br>Secure Sessions</div>
              <div class="feature">⚡<br>Fast Access</div>
              <div class="feature">🌐<br>Any Device</div>
          </div>

          <div class="info">
              <strong>✨ Features:</strong><br>
              • 24-hour secure sessions<br>
              • Mobile-optimized interface<br>
              • Real-time VPN monitoring<br>
              • One-click server control<br>
...
def get_vpn_status(session_id):
  ...
  try:
      item = mertrics_table.get_item(Key={'id': 'traffic_stats'})['Item']
  ...
  <body>
      <div class="container">
          <div class="header">
              <h1>🔒 Smart VPN Server Management</h1>
              <p>Free Tier Optimized • Auto-monitoring • Split Tunneling</p>
          </div>
          ...
          <div class="status-grid">
              ...
              <div class="status-card data-transfer">
                  <h3>📊 Data Transfer (This Month)</h3>
                  ...
                  {'<br><strong>⚠️ Approaching free tier limit!</strong>' if transfer_stats['status'] == 'warning' else ''}
                  {'<br><strong>🚨 Exceeded free tier limit!</strong>' if transfer_stats['status'] == 'critical' else ''}
              </div>
          </div>

          <div class="info">
              <strong>📈 VPN Traffic Stats:</strong><br>
              Last Check: {last_check}<br>
              Bytes Transferred: {bytes_transferred:,}<br>
              Auto-shutdown: 22:00 daily if inactive
          </div>

          {'<div class="split-tunnel-info"><strong>🔀 Split Tunneling Enabled</strong><br>Only specific traffic routes through VPN to minimize data usage. Perfect for accessing blocked sites while keeping regular browsing direct.</div>' if SPLIT_TUNNELING else ''}

          <div class="mobile-optimized">
              {'<a href="?action=stop&session=' + session_id + '" class="button stop-btn">🛑 Stop Server</a>' if state == 'running' else '<a href="?action=start&session=' + session_id + '" class="button">▶️Start Server</a>'}
              <a href="?action=config&session={session_id}" class="button">📱 Get Config</a>
              <a href="?action=status&session={session_id}" class="button">🔄 Refresh</a>
              <a href="?action=logout" class="button logout-btn">🚪 Logout</a>
          </div>

          <div class="info">
              <strong>💰 Cost Optimization:</strong><br>
              • Free for 12 months (t3.micro + 15GB transfer)<br>
              • Auto-shutdown saves ~50% costs<br>
              • Split tunneling reduces data usage by 70-90%<br>
              • After free tier: ~$7.59/month
          </div>

          <div class="info">
              <strong>🛡️ Security Features:</strong><br>
              • WireGuard encryption<br>
              • Automatic firewall configuration<br>
              • Traffic monitoring and alerts<br>
...
def check_traffic_and_shutdown():
    ...
    try:
        ...
        # Store traffic data
        mertrics_table.put_item(
            Item={
                'id': 'traffic_stats',
                'last_check': datetime.utcnow().isoformat(),
                'bytes_transferred': 0,  # Will be updated with actual data
                'data_transfer_gb': Decimal(str(transfer_stats['used_gb']))
            }
        )
        ...
        last_period_usage = cloudwatch.get_metric_statistics(
            Namespace='AWS/EC2',
            MetricName='NetworkOut',
            Dimensions=[{'Name': 'InstanceId', 'Value': INSTANCE_ID}],
        ...
        logger.info(f"Last 3 minutes traffic: {last_period_usage}")
        last_period_usage_num = sum([point['Sum'] for point in last_period_usage])
        if last_period_usage_num == 0:
            logger.info("No traffic in the last 3 minutes, shutting down the server to save costs.")
            ec2.stop_instances(InstanceIds=[INSTANCE_ID])
    except Exception as e:
